...
def plot_training_time_vs_model_arch(df, save_path=None):
    """
    Plot 5A: Training Time vs Model Architecture
    Double bar chart comparing training_time between CPU and MPS devices
    """
    ...
    y_max = plot_data['training_time'].max() * 1.2
    ...
    for i, device in enumerate(devices):
        ...
        for arch in model_archs:
            ...
            if not arch_data.empty:
                training_time.append(arch_data['training_time'].iloc[0])
            else:
        ...
        # Double bars
        bars = ax.bar(x_pos, training_time, width, 
                     color=cpu_color if device == 'cpu' else mps_color, 
                     alpha=0.8, label=f"device='{device}'")
        
        # Add value labels on the bars
        for j, value in enumerate(training_time):
            if value > 0:
                ax.text(x_pos[j], value + y_max*0.01, f'{value:.1f}', 
    ...
    ax.set_title('Model Scaling: Training Time vs Model Architecture\n(Batch size: 32, Block size: 64)', 
...
def plot_training_time_vs_batch_size(df, save_path=None):
    """
    Plot 5B: Training Time vs Batch Size
    Double bar chart comparing training_time between CPU and MPS devices
    """
    ...
    for i, device in enumerate(devices):
        ...
        if not device_data.empty:
            training_time = device_data['training_time'].values
            x_pos = x + i * width
...
def plot_training_time_vs_block_size(df, save_path=None):
    """
    Plot 5C: Training Time vs Block Size
    Double bar chart comparing training_time between CPU and MPS devices
    """
    # Focus on one representative model size
    plot_data = df[
    ...
    ax.set_ylabel('Training Time (seconds)', fontsize=12, fontweight='bold')
    ax.set_title('Training Time vs Block Size\n(Model: 4 layers, 4 heads, 256 embd, 32 batch)', 
                 fontsize=14, fontweight='bold')
...
fig_5a = plot_training_time_vs_model_arch(df, save_path='results/plot_5a_training_time_vs_model_arch.png')
fig_5b = plot_training_time_vs_batch_size(df, save_path='results/plot_5b_training_time_vs_batch_size.png')
fig_5c = plot_training_time_vs_block_size(df, save_path='results/plot_5c_training_time_vs_block_size.png')
